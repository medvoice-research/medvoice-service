...
# Main search logic
if patient_hash and len(patient_hash.strip()) >= 8 and (search_button or st.session_state.get('last_search') == patient_hash):
    ...
    try:
        ...
        response = api_client.get_patient_workflows(
            patient_hash=patient_hash.strip(),
            status=status_param,
            limit=limit,
        ...
        if total_count != filtered_count:
            st.info(f"ℹ️ Showing {filtered_count} out of {total_count} total workflows (filtered by status: {status_filter})")
...
st.caption("🔒 **Privacy**: Patient hashes are one-way cryptographic hashes. The original patient name cannot be recovered from the hash alone.")
