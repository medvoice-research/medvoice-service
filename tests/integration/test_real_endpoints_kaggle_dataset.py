...
class TestRealEndpointWithHIPAAFilenames:
    ...
    def test_speech_to_text_with_patient_cardiology(self, sample_audio_files):
        ...
        status_response = requests.get(
            f"{BASE_URL}/temporal/workflow/{workflow_id}",
            timeout=10
        )
        ...
        if status_response.status_code == 200:
            ...
            print(f"⏳ Waiting for Temporal to index workflow...")
            time.sleep(2)  # Give Temporal time to index the new workflow
            
            patient_workflows_response = requests.get(
                f"{BASE_URL}/temporal/patient/{patient_hash}/workflows",
                timeout=10
            )
            ...
            if patient_workflows_response.status_code == 200:
                ...
                assert workflow_id in workflow_ids, f"Workflow {workflow_id} not found in patient query. Found: {workflow_ids}"
    ...
    def test_speech_to_text_multiple_departments(self, sample_audio_files):
        ...
        for test_case in test_cases:
            ...
            if response.status_code == 200:
                ...
                assert f"pt_{patient_hash}" in workflow_id, \
                    f"Expected patient hash in workflow ID"
    ...
    def test_get_patient_latest_workflow(self):
        ...
        response = requests.get(
            f"{BASE_URL}/temporal/patient/{patient_hash}/latest",
            timeout=10
        )
    ...
    def test_get_patient_consultation_history(self):
        ...
        response = requests.get(
            f"{BASE_URL}/temporal/patient/{patient_hash}/consultations",
            timeout=10
        )
