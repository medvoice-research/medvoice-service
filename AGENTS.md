...
- **Temporal Activity Patterns**: Activities in [`activities.py`](app/activities.py:24) must use `@activity.defn` decorator. Error handling uses [`TemporalErrorHandler`](app/temporal_error_handler.py:14) to classify retryable vs non-retryable errors (401/auth = non-retryable, network/CUDA OOM = retryable)
