"""
Model download utility for WhisperX-FastAPI.

This module provides functions to safely download models from Hugging Face Hub
with proper error handling and retry logic, and a command-line interface
to pre-download models for WhisperX.
"""
...
# Handle imports properly based on execution context
try:
    # When running as a script file
    script_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.abspath(os.path.join(script_dir, ".."))
    sys.path.insert(0, parent_dir)
    from app.logger import logger
except (NameError, ImportError):
    # When running in interactive mode or as a standalone script
    import logging
    logging.basicConfig(
        level=logging.INFO,
...
def validate_huggingface_token(token: Optional[str] = None) -> bool:
    ...
    Args:
        token: Optional token to validate (uses env var if not provided)
    ...
    try:
        ...
        logger.warning(
            "Token validation failed with status code %d", 
            response.status_code
        )
        return False
    except requests.exceptions.RequestException as e:
        logger.warning("Error validating token: %s", str(e))
        return False


def preload_models(models: list, token: Optional[str] = None, cache_dir: Optional[str] = None) -> None:
    """
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Pre-download models for WhisperX API."
    )
    parser.add_argument(
        "--models", 
        nargs="+", 
        default=DEFAULT_MODELS,
        help="Space-separated list of model repository IDs to download"
    )
    parser.add_argument(
        "--token", 
    ...
    parser.add_argument(
        "--verify", 
        action="store_true", 
        help="Just verify the token without downloading models"
    )
    parser.add_argument(
        "--cache-dir",
...
def main():
    ...
    # Download models
    logger.info("Starting model downloads. This may take a while...")
    preload_models(args.models, token, args.cache_dir)
