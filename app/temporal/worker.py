...
async def main():
    """Start Temporal worker with robust retry logic for connection resilience."""
    max_retries = 5
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not client:
                raise ConnectionError("Failed to get Temporal client from manager")
            ...
            logger.info("=" * 70)
            logger.info("Worker connected successfully to Temporal server")
            ...
            # If we reach here, worker.run() completed normally (shutdown)
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(
                    f"Worker connection failed (attempt {attempt + 1}/{max_retries}): {e}. Retrying in {delay}s..."
                )
                await asyncio.sleep(delay)
            else:
                # Final attempt failed
                logger.error(f"Worker failed to connect after {max_retries} attempts. Last error: {e}")
                raise
        finally:
