...
async def get_patient_workflows(
    patient_hash: str, status: Optional[str] = Query(None, description="Filter by workflow status")
):
    """
    ...
    try:
        ...
        if not db_workflows:
            return {"patient_hash": patient_hash, "total_found": 0, "workflows": []}
        ...
        return {"patient_hash": patient_hash, "total_found": len(workflows), "workflows": workflows}
