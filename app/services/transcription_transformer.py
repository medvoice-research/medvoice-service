        ...
        try:
            ...
            if not speaker_mapping:
                ...
                return self._create_empty_result(
                    "No speakers detected in transcription",
                    workflow_id,
            ...
            full_transcript_plain = self.formatter.generate_transcript(
                dialogue_segments, 
                format="plain"
            )
            
            full_transcript_markdown = self.formatter.generate_transcript(
                dialogue_segments,
                format="markdown"
            )
            ...
            self.logger.info(
                f"Transformation complete: {len(dialogue_segments)} segments, "
                f"{len(speaker_mapping)} speakers"
            )
    ...
    def validate_transformation(self, transformed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "speaker_roles": {
                    sid: info.get("role") 
                    for sid, info in speaker_mapping.items()
                }
            }
        }
